...
function aiLogs(){try{const arr=JSON.parse(storageGet(AI_LOG_KEY,'[]')||'[]');return Array.isArray(arr)?arr:[]}catch(e){return []}}
function saveAiLogs(items){storageSet(AI_LOG_KEY,JSON.stringify(items.slice(-50)))}
function addAiLog(entry){const items=aiLogs();items.push({id:uid(),at:now(),undone:false,...entry});saveAiLogs(items);return items.at(-1)}
...
function aiWorkName(text=''){let t=aiStripLeadCommand(text);t=t.replace(/\s+(?:\d{1,3}(?:\s\d{3})+|\d+(?:[.,]\d+)?)\s*(?:тыс(?:яч[аиу]?|\.)?|руб(?:лей|ля|ль|\.)?|₽)?\s*$/i,'').trim();return t||'Новая работа'}
...
function parseAiCommand(text,targetKey=''){
  ...
  if(/(?:получил|получили|пришл[аои]|перевел|перевели|заказчик\s+(?:дал|отдал|перевел)|оплатил|оплата)/.test(norm)&&amount>0){return {ok:true,type:'payment',targetKey:guessed,amount,text:raw,summary:`Добавить оплату ${money(amount)}`}}
  if(/(?:отдал|заплатил|выплатил|расход)/.test(norm)&&/(?:посред|рабоч|мастер|субподряд|комисси)/.test(norm)&&amount>0){const who=/посред/.test(norm)?'Посредник':/субподряд/.test(norm)?'Субподрядчик':/рабоч/.test(norm)?'Рабочий':'Исполнитель';return {ok:true,type:'expense',targetKey:guessed,amount,worker:who,text:raw,summary:`Добавить расход ${money(amount)} · ${who}`}}
  if(/(?:купил|купили|закупил|закупили|материал)/.test(norm)&&amount>0){let name=raw.replace(/^.*?(?:купил|купили|закупил|закупили)\s*/i,'').replace(/\s+(?:за|на)\s+\d+[\d\s.,]*(?:руб\w*|₽|тыс\w*)?.*$/i,'').trim();if(!name||/^материал/i.test(name))name='Материалы / расходники';return {ok:true,type:'purchase',targetKey:guessed,amount,name,text:raw,summary:`Добавить покупку ${money(amount)} · ${name}`}}
  if(/(?:закончил|закончена|завершил|завершена|выполнил|выполнена|готово)/.test(norm)){const row=aiFindWork(target.order,raw);if(row)return {ok:true,type:'work_complete',targetKey:guessed,rowId:row.id,rowName:row.name,previousPct:workProgressPct(row),text:raw,summary:`Отметить «${row.name}» выполненной на 100%`};return {ok:false,error:'Не нашла подходящую работу в выбранном заказе. Можно уточнить название.'}}
  if(/(?:добавь|добавить|новая работа|допработ)/.test(norm)){const work=aiParseWorkAdd(raw);if(work&&parseNum(work.price)>0)return {ok:true,type:'work_add',targetKey:guessed,work,text:raw,summary:`Добавить работу «${work.name}» · ${work.qty} ${work.unit} × ${money(parseNum(work.price))}`};return {ok:false,error:'Не поняла цену новой работы. Например: «Добавь монтаж подсветки 3500».'}}
  if(/(?:заметка|запиши|комментарий)/.test(norm)){const note=raw.replace(/^.*?(?:заметка|запиши|комментарий)[:\s-]*/i,'').trim();if(note)return {ok:true,type:'order_note',targetKey:guessed,note,text:raw,summary:`Добавить заметку к заказу: «${note}»`}}
  return {ok:false,error:'Пока не уверена, что именно нужно изменить. Попробуйте сказать: «получил 20 тысяч», «добавь монтаж подсветки 3500», «купил клей за 4000» или «закончил плинтус».'};
}
function aiActionLabel(type){return {payment:'Оплата',expense:'Расход',purchase:'Покупка',work_add:'Новая работа',work_complete:'Выполнение',order_note:'Заметка'}[type]||'Изменение'}
...
function aiDraftHtml(draft){if(!draft)return '<div class="aiEmpty"><strong>FRAME ждёт команду</strong><span>Сначала выберите заказ, затем напишите или продиктуйте, что произошло.</span></div>';if(!draft.ok)return `<div class="aiError"><strong>Нужно уточнение</strong><span>${esc(draft.error)}</span></div>`;return `<div class="aiPreview"><div class="aiPreviewHead"><span>${esc(aiActionLabel(draft.type))}</span><b>Проверка перед записью</b></div><strong>${esc(draft.summary)}</strong><small>${esc(aiTargetLabel(draft.targetKey))}</small><div class="aiPreviewActions"><button id="applyAiDraftBtn" class="btn primary">✓ Применить</button><button id="clearAiDraftBtn" class="btn ghost">Изменить фразу</button></div></div>`}
...
async function applyAiDraft(){const d=aiDraft;if(!d?.ok)return;const target=aiTargetByKey(d.targetKey);if(!target){toast('Заказ не найден');return}const {object,order}=target;let undo={};
  ...
  if(d.type==='purchase'){const item=normalizePurchase({name:d.name||'Материалы / расходники',amount:String(d.amount),date:today(),status:'due',comment:`FRAME AI: ${d.text}`});order.purchases.push(item);undo={kind:'remove_purchase',id:item.id}}
  if(d.type==='work_add'){const oldContract=String(order.pricing.contractTotal??'').trim();const row=normalizeGenericRow({...d.work,moduleId:inferWorkModuleId('',d.work.name),group:workModuleTitle(inferWorkModuleId('',d.work.name)),comment:`Добавлено через FRAME AI · ${today()}`},'Работы');order.works.push(row);if(oldContract!=='')order.pricing.contractTotal=String(parseNum(oldContract)+workRowTotal(row));undo={kind:'remove_work',id:row.id,contractTotal:oldContract}}
  if(d.type==='work_complete'){const row=order.works.find(x=>x.id===d.rowId);if(!row){toast('Работа больше не найдена');return}undo={kind:'restore_progress',id:row.id,pct:workProgressPct(row),note:row.progressNote||''};row.progressPct=100;row.progressNote=`FRAME AI · ${today()}`}
  if(d.type==='order_note'){undo={kind:'restore_comment',comment:order.comment||''};order.comment=[order.comment,d.note].filter(Boolean).join(' · ')}
  await saveObject(object);addAiLog({type:d.type,targetKey:d.targetKey,text:d.text,summary:d.summary,undo});aiDraft=null;routeState.aiTarget=d.targetKey;render();toast('FRAME обновил заказ')}
async function undoLastAiAction(){const logs=aiLogs(),idx=[...logs].map((x,i)=>({x,i})).reverse().find(y=>!y.x.undone)?.i;if(idx===undefined)return;const log=logs[idx],target=aiTargetByKey(log.targetKey);if(!target){toast('Исходный заказ не найден');return}const {object,order}=target,u=log.undo||{};
  if(u.kind==='remove_payment')order.payments=order.payments.filter(x=>x.id!==u.id);
  if(u.kind==='remove_expense')order.expenses=order.expenses.filter(x=>x.id!==u.id);
  if(u.kind==='remove_purchase')order.purchases=order.purchases.filter(x=>x.id!==u.id);
  if(u.kind==='remove_work'){order.works=order.works.filter(x=>x.id!==u.id);if(u.contractTotal!==undefined)order.pricing.contractTotal=u.contractTotal}
  if(u.kind==='restore_progress'){const row=order.works.find(x=>x.id===u.id);if(row){row.progressPct=u.pct||0;row.progressNote=u.note||''}}
  if(u.kind==='restore_comment')order.comment=u.comment||'';
  await saveObject(object);logs[idx].undone=true;saveAiLogs(logs);aiDraft=null;render();toast('Последнее AI-изменение отменено')}
function bindAiView(){const select=$('aiTargetSelect');if(select)select.onchange=e=>{routeState.aiTarget=e.target.value;aiDraft=null;$('aiResult').innerHTML=aiDraftHtml(null)};if($('aiAnalyzeBtn'))$('aiAnalyzeBtn').onclick=()=>{const text=$('aiCommandInput').value;aiDraft=parseAiCommand(text,routeState.aiTarget);if(aiDraft?.targetKey&&aiDraft.targetKey!==routeState.aiTarget){routeState.aiTarget=aiDraft.targetKey;if(select)select.value=aiDraft.targetKey}$('aiResult').innerHTML=aiDraftHtml(aiDraft);bindAiResult()};if($('aiMicBtn'))$('aiMicBtn').onclick=startAiVoice;$$('[data-ai-example]').forEach(b=>b.onclick=()=>{$('aiCommandInput').value=b.dataset.aiExample;$('aiCommandInput').focus()});if($('undoAiBtn'))$('undoAiBtn').onclick=undoLastAiAction;bindAiResult()}
...
function renderDashboard(){
  ...
  return `<section class="view"><div class="card"><h1>Заказы</h1><p class="help">Работы, деньги и документы без лишних уровней меню.</p><div class="stats"><button class="stat statButton" data-dashboard-list="all"><span>Заказы</span><strong>${entries.length}</strong></button><button class="stat statButton" data-dashboard-list="active"><span>Активные</span><strong>${active}</strong></button><button class="stat statButton wideStat" data-dashboard-list="budget"><span>Стоимость работ</span><strong>${money(total)}</strong></button></div><button id="monthFinanceCard" class="monthGoalCard" type="button"><div class="monthGoalTop"><span><small>${esc(range.label)}</small><strong>Чистыми ${money(finance.net)}</strong></span><span class="monthGoalRemain">Осталось ${money(remain)}</span></div><div class="goalTrack"><i style="width:${pct}%"></i></div><div class="monthGoalMeta"><span>Цель ${money(goal)}</span><span>Получено ${money(finance.received)} · расходы ${money(finance.spent)}</span></div></button><button id="frameAiBtn" class="frameAiLaunch" type="button"><span class="frameAiOrb">✦</span><span><strong>FRAME AI <em>BETA</em></strong><small>Расскажите, что произошло — FRAME подготовит изменение</small></span><b>›</b></button><div class="actions dashboardActions"><button id="newObjectBtn" class="btn primary">＋ Новый заказ</button><button id="documentsBtn" class="btn ghost">▤ Документы</button><button id="quickPriceBtn" class="btn ghost">₽ Прайс-лист</button></div></div></section>`;
}
...
function renderOrderView(){
  ...
  return `<section class="view">
    ...
    <div class="card" id="orderActionsCard">
      <div class="sectionTitle"><div><h2>Что делаем?</h2><p class="help compact">Работы собраны по разделам. Откройте нужный модуль или весь чек-лист.</p></div><button id="orderAiBtn" class="btn aiMiniBtn">✦ AI</button></div>
...
function bindRoute(){
  ...
  if(route==='dashboard'){
    if($('frameAiBtn'))$('frameAiBtn').onclick=()=>{aiDraft=null;navigate('ai',{aiTarget:aiDefaultTargetKey()})};
...
function bindOrderView(){const object=currentObject(),order=currentOrder();if(!object||!order)return;if($('orderAiBtn'))$('orderAiBtn').onclick=()=>{aiDraft=null;navigate('ai',{aiTarget:`${object.id}|${order.id}`})};
