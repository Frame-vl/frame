...
const DB_VERSION=2;
const STORE='objects';
const BACKUP_KEY='frameObjectsBackupV170';
const OLD_BACKUP_KEYS=['frameObjectsBackupV160','frameObjectsBackupV150','frameObjectsBackupV140','frameObjectsV140'];
const RATE_KEY='frameRatesV170';
const OLD_RATE_KEYS=['frameRatesV160'];
const PROFILE_KEY='frameProfileV170';
const EXPORT_PRIVATE_KEY='frameExportPrivateV170';
...
const DEFAULT_RATES={
  ...
  'door.service.demo':1500,'door.service.lock.single':3000,'door.service.lock.series':2000,'door.service.trim':2000,
  'door.service.adjustment':3000,'door.service.limiter':500,'door.service.dobor':2000,'door.service.hinges':0,
...
const RATE_GROUPS={
  ...
  doors:[
    {title:'Установка двери',items:[['Базовая межкомнатная дверь','door.install.interroom.base','шт.'],['Комплекс с доборами и наличниками','door.install.interroom.complex','шт.'],['Откатная дверь','door.install.sliding','шт.'],['Дверь-пенал','door.install.pocket','шт.'],['Двустворчатая дверь','door.install.double','шт.'],['Скрытая распашная дверь','door.install.hidden','шт.']]},
    {title:'Обслуживание и доработка',items:[['Демонтаж старой двери','door.service.demo','шт.'],['Установка одного замка / отдельный выезд','door.service.lock.single','шт.'],['Установка нескольких замков','door.service.lock.series','шт.'],['Замена телескопических наличников','door.service.trim','компл.'],['Регулировка двери','door.service.adjustment','шт.'],['Установка ограничителя','door.service.limiter','шт.'],['Замена доборов','door.service.dobor','компл.'],['Нестандартные петли','door.service.hinges','компл.']]},
...
function loadProfile(){try{return {...DEFAULT_PROFILE,...JSON.parse(storageGet(PROFILE_KEY,'{}')||'{}')}}catch(e){return {...DEFAULT_PROFILE}}}
function saveProfile(){storageSet(PROFILE_KEY,JSON.stringify(profile))}
function loadRates(){let saved={};for(const key of [RATE_KEY,...OLD_RATE_KEYS]){try{const value=JSON.parse(storageGet(key,'{}')||'{}');if(value&&typeof value==='object')saved={...saved,...value}}catch(e){}}
  const mapped={
    ...
    'door.service.demo':saved['door.demo'],'door.service.lock.single':saved['door.lock.single'],
    'door.service.lock.series':saved['door.lock.series'],'door.service.limiter':saved['door.limiter'],
...
function defaultOrder(index=1){return {id:uid(),title:`Заказ ${index}`,date:today(),status:'draft',comment:'',floor:defaultFloor(),doors:defaultDoors(),purchases:[],payments:[],stages:[],photos:[],documentHistory:[]}}
function defaultObject(){return {id:uid(),version:VERSION,createdAt:now(),updatedAt:now(),contact:{name:'',phone:'',address:'',comment:''},orders:[defaultOrder(1)],legacyMeta:{}}}
...
function floorRateKey(f){const p=['straight','eng','fr','corner','complex'].includes(f.pattern)?f.pattern:'complex';if(f.cover==='spc')return `floor.spc.${p}`;if(f.cover==='laminate')return `floor.${f.thickness==='12plus'?'laminate12':'laminate'}.${p}`;if(f.cover==='glue')return `floor.glue.${p}`;return ''}
function normalizeFloor(raw={}){
  if(raw&&Array.isArray(raw.demolition)&&Array.isArray(raw.preparation)&&Array.isArray(raw.extras)){
    ...
    out.area=String(raw.area??'');out.installRate=String(raw.installRate??rateForFloor(out.cover,out.thickness,out.pattern));
  ...
  f.thickness=raw.thickness||'standard';f.installRate=String(raw.installRate??rateForFloor(f.cover,f.thickness,f.pattern));
...
function doorServicePreset(type='adjustment'){
  const map={
    lock:['Установка / замена замка','шт.','door.service.lock.single'],trim:['Замена телескопических наличников','компл.','door.service.trim'],
    adjustment:['Регулировка двери','шт.','door.service.adjustment'],limiter:['Установка ограничителя','шт.','door.service.limiter'],
    dobor:['Замена доборов','компл.','door.service.dobor'],hinges:['Нестандартные петли','компл.','door.service.hinges'],
...
function normalizeDoorItem(item={}){
  ...
  if(item.kind==='service'){const x={...doorServicePreset(item.serviceType||'other'),...item};x.qty=String(item.qty??1);x.unitPrice=String(item.unitPrice??item.price??'');return x}
...
function normalizePurchase(p={}){return {id:p.id||uid(),name:p.name||'',amount:String(p.amount??''),date:p.date||today(),status:p.status||'due',comment:p.comment||''}}
...
function normalizeOrder(raw={},index=1){return {
  id:raw.id||uid(),title:raw.title||`Заказ ${index}`,date:raw.date||today(),status:raw.status||'draft',comment:raw.comment||'',
  floor:normalizeFloor(raw.floor||{}),doors:normalizeDoors(raw.doors||{}),purchases:(raw.purchases||[]).map(normalizePurchase),
  payments:(raw.payments||[]).map(normalizePayment),stages:(raw.stages||[]).map(normalizeStage),photos:raw.photos||[],documentHistory:raw.documentHistory||[]
}}
...
function normalizeObject(raw={}){
  ...
  const out={id:base.id||uid(),version:VERSION,createdAt:base.createdAt||base.updatedAt||now(),updatedAt:base.updatedAt||now(),contact:{name:c.name||'',phone:c.phone||'',address:c.address||'',comment:c.comment||''},orders:(base.orders||[]).map((o,i)=>normalizeOrder(o,i+1)),legacyMeta:base.legacyMeta||{}};
...
function stripPhotos(object){const c=clone(object);for(const order of c.orders||[])order.photos=(order.photos||[]).map(p=>({id:p.id,caption:p.caption||'',missing:true}));return c}
...
function queueSave(label='Сохранено'){clearTimeout(saveTimer);const object=currentObject();if(!object)return;saveTimer=setTimeout(async()=>{try{await saveObject(object);const el=$('autosaveStatus');if(el){el.textContent=label;setTimeout(()=>{if(el.textContent===label)el.textContent=''},1500)}}catch(e){console.error(e);toast('Не удалось сохранить изменения')}},500)}

function floorCoverName(f){return {spc:'SPC / замковый кварцвинил',laminate:f.thickness==='12plus'?'Ламинат 12 мм и толще':'Ламинат',glue:'Клеевой кварцвинил / LVT',other:f.otherCover||'Другое покрытие'}[f.cover]||'Напольное покрытие'}
...
function orderWorkTotal(order){const f=calculateFloor(order.floor),d=calculateDoors(order.doors);return f.workTotal+d.total}
function orderPaid(order){return (order.payments||[]).reduce((s,p)=>s+parseNum(p.amount),0)}
function orderRemaining(order){return Math.max(0,orderWorkTotal(order)-orderPaid(order))}
function orderDuePurchases(order){return (order.purchases||[]).filter(p=>p.status!=='reimbursed').reduce((s,p)=>s+parseNum(p.amount),0)}
function objectTotals(object){return (object.orders||[]).reduce((acc,o)=>{acc.work+=orderWorkTotal(o);acc.paid+=orderPaid(o);acc.due+=orderDuePurchases(o);return acc},{work:0,paid:0,due:0})}
...
function navigate(next,state={}){closeSheet();route=next;routeState=state;window.scrollTo({top:0,behavior:'smooth'});render()}
function render(){const host=$('viewHost');const renderers={dashboard:renderDashboard,settings:renderSettings,globalDocs:renderGlobalDocs,object:renderObjectView,order:renderOrderView,floor:renderFloorView,doors:renderDoorsView,purchases:renderPurchasesView,payments:renderPaymentsView,photos:renderPhotosView,documents:renderDocumentsView};host.innerHTML=(renderers[route]||renderDashboard)();bindRoute();}
...
function renderDashboard(){
  const total=objects.reduce((s,o)=>s+objectTotals(o).work,0),active=objects.filter(o=>o.orders.some(r=>['agreed','work'].includes(r.status))).length;
...
function renderPriceSettings(){return `<div class="card"><h2>Мои цены</h2><p class="help">Стартовые цены уже заполнены. Меняйте их постепенно. Новые ставки применяются только к новым расчётам и не пересчитывают старые заказы.</p>
  <details open><summary>Полы</summary>${RATE_GROUPS.floor.map(settingsRateSection).join('')}</details>
  <details><summary>Двери</summary>${RATE_GROUPS.doors.map(settingsRateSection).join('')}</details>
...
function renderGlobalDocs(){return `<section class="view"><div class="actions pageBack"><button class="btn ghost" data-go="dashboard">← Объекты</button></div>
  <div class="card"><h1>Быстрые документы</h1><p class="help">Прайс-лист и пустой договор можно сформировать без создания объекта.</p>
    <div class="modules"><button class="module" id="openPriceList"><div class="moduleTop"><span class="moduleIcon">₽</span><span class="moduleStatus">PDF</span></div><h3>Прайс-лист</h3><p>Выберите направления и отправьте клиенту конкретные ориентиры.</p></button><button class="module" id="blankContract"><div class="moduleTop"><span class="moduleIcon">▤</span><span class="moduleStatus">A4</span></div><h3>Пустой договор</h3><p>Бланк для печати и заполнения на объекте.</p></button></div>
  </div></section>`}
...
function renderObjectView(){const object=currentObject();if(!object)return renderDashboard();const totals=objectTotals(object),remaining=Math.max(0,totals.work-totals.paid),related=relatedObjectsCount(object);return `<section class="view">
  ...
  <div class="card"><div class="objectHero"><div><h1>${esc(object.contact.address||'Новый объект')}</h1><p class="help compact">${esc(object.contact.name||'Заказчик не указан')} ${object.contact.phone?'· '+esc(formatPhone(object.contact.phone)):''}</p></div>
    <div class="moneyStack"><div class="moneyMetric primary"><span>Работы</span><strong>${money(totals.work)}</strong></div><div class="moneyMetric"><span>Оплачено</span><strong>${money(totals.paid)}</strong></div><div class="moneyMetric"><span>Осталось</span><strong>${money(remaining)}</strong></div></div></div>
    ...
    <h2>Карточка объекта</h2><div class="grid two"><label>Заказчик<input id="objectName" value="${esc(object.contact.name)}" placeholder="Имя или ФИО"></label><label>Телефон<input id="objectPhone" inputmode="tel" value="${esc(formatPhone(object.contact.phone))}" placeholder="+7 (___) ___-__-__"></label><label class="wide">Адрес<input id="objectAddress" value="${esc(object.contact.address)}" placeholder="Город, улица, дом, квартира"></label><label class="wide">Комментарий<input id="objectComment" value="${esc(object.contact.comment)}" placeholder="Необязательно"></label></div>
    ${related?`<div class="relatedHint">У этого контакта есть ещё объектов: ${related}</div>`:''}<div id="autosaveStatus" class="autosave"></div>
  </div>
...
function renderOrderView(){const order=currentOrder();if(!order)return renderObjectView();const work=orderWorkTotal(order),paid=orderPaid(order),remaining=orderRemaining(order),due=orderDuePurchases(order);return `<section class="view">
  ...
  <div class="card"><div class="objectHero"><div><h1>${esc(order.title)}</h1><p class="help compact">${ruDate(order.date)} · ${orderStatusName(order.status)}</p></div><div class="moneyStack"><div class="moneyMetric primary"><span>Работы</span><strong>${money(work)}</strong></div><div class="moneyMetric"><span>Оплачено</span><strong>${money(paid)}</strong></div><div class="moneyMetric"><span>Осталось</span><strong>${money(remaining)}</strong></div></div></div>
    ${due?`<div class="dangerNote"><strong>К возмещению: ${money(due)}</strong></div>`:''}
    <div class="grid two"><label>Название заказа<input id="orderTitle" value="${esc(order.title)}"></label><label>Дата<input id="orderDate" type="date" value="${esc(order.date)}"></label><label>Статус<select id="orderStatus">${[['draft','Черновик'],['agreed','Согласован'],['work','В работе'],['done','Завершён'],['paid','Оплачен']].map(([v,n])=>`<option value="${v}" ${order.status===v?'selected':''}>${n}</option>`).join('')}</select></label><label>Комментарий<input id="orderComment" value="${esc(order.comment)}" placeholder="Необязательно"></label></div><div id="autosaveStatus" class="autosave"></div>
  </div>
  <div class="card"><h2>Что нужно сделать</h2><p class="help">Открывайте только нужный раздел. Остальное не мешает.</p><div class="modules">
    ${moduleCard('floor','▱','Полы','Покрытие, плинтус, демонтаж, основание.',order.floor.completed?'Заполнено':'Не заполнено',order.floor.completed)}
    ${moduleCard('doors','▯','Двери','Установка, обслуживание и оформление проёмов.',order.doors.completed?`${order.doors.items.length} поз.`:'Не заполнено',order.doors.completed)}
    ${moduleCard('purchases','🧾','Покупки мастера','Материалы, купленные за свои деньги.',due?`К возмещению ${money(due)}`:(order.purchases.length?'Возмещено':'Нет покупок'),order.purchases.length>0&&!due)}
    ${moduleCard('payments','₽','Оплаты','Оплата по завершении или этапы.',paid?`Оплачено ${money(paid)}`:'Не оплачено',paid>=work&&work>0)}
    ${moduleCard('photos','▧','Фото','Локальные фотографии с подписями.',order.photos.length?`${order.photos.length} фото`:'Нет фото',order.photos.length>0)}
    ${moduleCard('documents','▤','Документы','КП, перечень, акт и договор.','Сформировать',false)}
  </div></div></section>`}
...
function renderFloorView(){const order=currentOrder();if(!order)return renderOrderView();const f=order.floor,calc=calculateFloor(f);const otherCover=f.cover==='other'?`<label class="wide">Название покрытия<input id="floorOtherCover" value="${esc(f.otherCover)}" placeholder="Например, инженерная доска"></label>`:'';const thickness=f.cover==='laminate'?`<label>Толщина<select id="floorThickness"><option value="standard" ${f.thickness!=='12plus'?'selected':''}>До 12 мм</option><option value="12plus" ${f.thickness==='12plus'?'selected':''}>12 мм и толще</option></select></label>`:'';
  ...
  return `<section class="view"><div class="actions pageBack"><button class="btn ghost" data-go="order">← Заказ</button></div><div class="card"><h1>Полы</h1><p class="help">Открыт только текущий вопрос. Заполненные шаги сворачиваются в короткие строки.</p><div class="stepList">${floorStepCard(f,0,'Покрытие',step0)}${floorStepCard(f,1,'Площадь',step1)}${floorStepCard(f,2,'Плинтус',step2)}${floorStepCard(f,3,'Демонтаж',step3)}${floorStepCard(f,4,'Подготовка основания',step4)}${floorStepCard(f,5,'Дополнительные работы',step5)}${floorStepCard(f,6,'Проверка',step6)}</div><div id="autosaveStatus" class="autosave"></div></div></section>`;
}

function installationCard(item,index){const min=item.type==='interroom'?parseNum(rates['door.install.interroom.base']):0;const below=min&&parseNum(item.unitPrice)<min;return `<article class="doorCard"><div class="rowHead"><div><h3>${esc(doorTypeName(item.type))}</h3><span class="contextTag">Комплексная установка</span></div><strong data-door-sum="${index}">${money(parseNum(item.qty)*parseNum(item.unitPrice))}</strong></div>
  ...
  ${item.type==='interroom'?`<label>Доборы<select data-door-index="${index}" data-door-key="dobor"><option value="none" ${item.dobor==='none'?'selected':''}>Без доборов</option><option value="standard" ${item.dobor==='standard'?'selected':''}>Доборы + наличники</option><option value="wide" ${item.dobor==='wide'?'selected':''}>Широкий / наборный добор</option><option value="other" ${item.dobor==='other'?'selected':''}>Другое</option></select></label>${['wide','other'].includes(item.dobor)?`<label>Доплата за доборы<input class="decimal" inputmode="decimal" data-door-index="${index}" data-door-key="doborExtra" value="${esc(item.doborExtra)}"></label>`:''}
...
function renderPaymentsView(){const order=currentOrder();if(!order)return renderOrderView();const work=orderWorkTotal(order),paid=orderPaid(order),remaining=orderRemaining(order);return `<section class="view"><div class="actions pageBack"><button class="btn ghost" data-go="order">← Заказ</button></div><div class="card"><h1>Оплаты</h1><p class="help">Без этапов по умолчанию. Новый платёж сразу предлагает оставшуюся сумму.</p><div class="summaryGrid"><div class="reviewSummary"><span>Работы</span><strong>${money(work)}</strong></div><div class="reviewSummary secondary"><span>Оплачено</span><strong id="paidTotal">${money(paid)}</strong></div><div class="reviewSummary secondary"><span>Осталось</span><strong id="remainingTotal">${money(remaining)}</strong></div></div>
  ...
  <details><summary>Этапы работ</summary><p class="help">Добавляйте только когда заказ действительно разбит на этапы.</p><div class="rowList">${order.stages.length?order.stages.map((s,i)=>`<div class="rowCard"><div class="rowGrid compact"><label>Этап<input data-stage-index="${i}" data-stage-key="name" value="${esc(s.name)}"></label><label>Сумма<input class="decimal" inputmode="decimal" data-stage-index="${i}" data-stage-key="amount" value="${esc(s.amount)}" placeholder="Сумма"></label><label>Статус<select data-stage-index="${i}" data-stage-key="paid"><option value="false" ${!s.paid?'selected':''}>Не оплачен</option><option value="true" ${s.paid?'selected':''}>Оплачен</option></select></label><button class="btn danger small" data-remove-stage="${i}">×</button></div></div>`).join(''):'<div class="empty">Этапов нет.</div>'}</div><button id="addStageBtn" class="btn ghost wide" style="margin-top:9px">＋ Добавить этап</button></details><div id="autosaveStatus" class="autosave"></div></div></section>`}

function renderPhotosView(){const order=currentOrder();if(!order)return renderOrderView();return `<section class="view"><div class="actions pageBack"><button class="btn ghost" data-go="order">← Заказ</button></div><div class="card"><h1>Фото объекта</h1><p class="help">Фотографии уменьшаются перед сохранением и остаются только на этом устройстве.</p><label class="btn primary fileBtn">＋ Добавить фотографии<input id="photoInput" type="file" accept="image/*" multiple hidden></label><div class="photoGrid">${order.photos.length?order.photos.map((p,i)=>`<div class="photo"><img src="${p.data||''}" alt=""><button data-remove-photo="${i}">×</button><input data-photo-index="${i}" value="${esc(p.caption||'')}" placeholder="Подпись"></div>`).join(''):'<div class="empty wide">Фотографий пока нет.</div>'}</div><div id="autosaveStatus" class="autosave"></div></div></section>`}

function renderDocumentsView(){const order=currentOrder();if(!order)return renderOrderView();return `<section class="view"><div class="actions pageBack"><button class="btn ghost" data-go="order">← Заказ</button></div><div class="card"><h1>Документы заказа</h1><p class="help">Перед клиентским PDF можно проверить и поправить строки. Внутренние подсказки в документ не попадут.</p><div class="modules">
...
function bindRoute(){
  ...
  if(route==='globalDocs'){
    ...
    $('blankContract').onclick=()=>showContractOptions(null);
  }
...
function bindObjectView(){const object=currentObject();if(!object)return;
  ...
  $('newOrderBtn').onclick=()=>{const order=defaultOrder(object.orders.length+1);object.orders.push(order);currentOrderId=order.id;queueSave();navigate('order');toast('Новый заказ создан')};
  ...
  $('deleteObjectBtn').onclick=async()=>{if(!confirm('Удалить объект вместе со всеми заказами?'))return;await dbDelete(object.id);objects=objects.filter(o=>o.id!==object.id);mirrorBackup();currentObjectId='';currentOrderId='';navigate('dashboard');toast('Объект удалён')};
}
...
function bindOrderView(){const object=currentObject(),order=currentOrder();if(!object||!order)return;
  ...
  $('deleteOrderBtn').onclick=()=>{if(object.orders.length===1){toast('У объекта должен остаться хотя бы один заказ');return}if(!confirm('Удалить этот заказ?'))return;object.orders=object.orders.filter(o=>o.id!==order.id);currentOrderId=object.orders[object.orders.length-1].id;queueSave();navigate('object');toast('Заказ удалён')};
}
...
function bindFloorView(){const order=currentOrder(),f=order?.floor;if(!order||!f)return;
  ...
  if($('floorCover'))$('floorCover').onchange=e=>{f.cover=e.target.value;if(f.cover!=='laminate')f.thickness='standard';autoRate();queueSave();render()};
  ...
  if($('finishFloorBtn'))$('finishFloorBtn').onclick=()=>{f.completed=calculateFloor(f).rows.length>0;queueSave();navigate('order');toast('Раздел «Полы» сохранён')};
}
...
function bindDoorsView(){const order=currentOrder(),d=order?.doors;if(!order||!d)return;
  ...
  $$('[data-door-index]').forEach(el=>el.addEventListener(el.tagName==='SELECT'?'change':'input',()=>{const i=+el.dataset.doorIndex,key=el.dataset.doorKey,item=d.items[i];if(!item)return;const isNumeric=['qty','unitPrice','doborExtra','lockExtra','hingeExtra'].includes(key);item[key]=isNumeric?rawNum(el.value):el.value;
    ...
    if(item.kind==='service'&&item.serviceType==='lock'&&key==='qty'&&item.priceMode!=='manual')item.unitPrice=String(rates[parseNum(item.qty)>1?'door.service.lock.series':'door.service.lock.single']||'');
  ...
  $$('[data-remove-door]').forEach(b=>b.onclick=()=>{d.items.splice(+b.dataset.removeDoor,1);d.completed=d.items.length>0;queueSave();render()});
  $$('[data-duplicate-door]').forEach(b=>b.onclick=()=>{const copy=clone(d.items[+b.dataset.duplicateDoor]);copy.id=uid();copy.qty='1';d.items.push(copy);d.completed=true;queueSave();render();setTimeout(()=>window.scrollTo({top:document.body.scrollHeight,behavior:'smooth'}),20)});
...
function showAddDoorSheet(){const html=`<div class="sectionTitle"><div><h1>Добавить дверную работу</h1><p class="help compact">Выберите понятный сценарий. Всё остаётся в одном окне.</p></div><button class="btn ghost small" data-close-sheet>Закрыть</button></div>
  <div class="choiceGrid" id="doorScenarioChoices"><button class="optionCard selected" data-door-scenario="installation"><strong>Установка двери</strong><small>Новая дверь или группа одинаковых дверей.</small></button><button class="optionCard" data-door-scenario="service"><strong>Обслуживание и доработка</strong><small>Замок, наличники, регулировка и другое.</small></button><button class="optionCard" data-door-scenario="opening"><strong>Оформление проёма</strong><small>Портал или проём входной двери.</small></button></div><div id="doorScenarioPanel" style="margin-top:12px"></div>`;openSheet(html);let scenario='installation';const renderScenario=()=>{const panel=$('doorScenarioPanel');if(scenario==='installation')panel.innerHTML=`<div class="list">${[['interroom','Межкомнатная дверь','Базовая от 6 000 ₽'],['sliding','Откатная дверь','Вдоль стены'],['pocket','Дверь-пенал','Полотно уходит в стену'],['double','Двустворчатая дверь','Два полотна'],['hidden','Скрытая распашная дверь','Алюминиевая коробка'],['entrance','Входная дверь','Цена вручную'],['other','Другое','Своя дверь']].map(([v,n,s])=>`<button class="optionCard" data-add-door-kind="installation" data-add-door-value="${v}"><strong>${n}</strong><small>${s}</small></button>`).join('')}</div>`;if(scenario==='service')panel.innerHTML=`<div class="list">${[['lock','Установка / замена замка','3 000 ₽ за отдельный выезд'],['trim','Замена телескопических наличников','2 000 ₽'],['adjustment','Регулировка двери','3 000 ₽'],['limiter','Установка ограничителя','500 ₽'],['dobor','Замена доборов','2 000 ₽'],['hinges','Нестандартные петли','Цена вручную'],['demo','Демонтаж старой двери','1 500 ₽'],['other','Другое','Своя работа']].map(([v,n,s])=>`<button class="optionCard" data-add-door-kind="service" data-add-door-value="${v}"><strong>${n}</strong><small>${s}</small></button>`).join('')}</div>`;if(scenario==='opening')panel.innerHTML=`<div class="list">${[['portal','Обычный портал','Цена вручную'],['entrancePortal','Проём входной двери','Ориентир 5 000 ₽'],['other','Другое','Нестандартный проём']].map(([v,n,s])=>`<button class="optionCard" data-add-door-kind="opening" data-add-door-value="${v}"><strong>${n}</strong><small>${s}</small></button>`).join('')}</div>`;$$('[data-add-door-kind]',panel).forEach(b=>b.onclick=()=>{const order=currentOrder();if(!order)return;let item;if(b.dataset.addDoorKind==='installation')item=defaultDoorInstallation(b.dataset.addDoorValue);else if(b.dataset.addDoorKind==='service')item=doorServicePreset(b.dataset.addDoorValue);else item=doorOpeningPreset(b.dataset.addDoorValue);if(item.kind==='installation'&&item.type==='other'){item.type='other';item.unitPrice='';item.priceMode='manual'}order.doors.items.push(item);order.doors.completed=true;queueSave();closeSheet();render();setTimeout(()=>window.scrollTo({top:document.body.scrollHeight,behavior:'smooth'}),20)});};
  $$('[data-door-scenario]',$('sheetPanel')).forEach(b=>b.onclick=()=>{scenario=b.dataset.doorScenario;$$('[data-door-scenario]',$('sheetPanel')).forEach(x=>x.classList.toggle('selected',x===b));renderScenario()});renderScenario();
}
...
function bindPurchasesView(){const order=currentOrder();if(!order)return;
  ...
  $('addPurchaseBtn').onclick=()=>{order.purchases.push(normalizePurchase({name:'',amount:'',date:today(),status:'due'}));queueSave();render();setTimeout(()=>window.scrollTo({top:document.body.scrollHeight,behavior:'smooth'}),20)};
}
function bindPaymentsView(){const order=currentOrder();if(!order)return;
  ...
  $('addPaymentBtn').onclick=()=>{const remaining=orderRemaining(order);order.payments.push(normalizePayment({amount:remaining?String(Math.round(remaining*100)/100):'',date:today(),note:'Оплата работ'}));queueSave();render();setTimeout(()=>window.scrollTo({top:document.body.scrollHeight,behavior:'smooth'}),20)};
...
function renderReviewSheet(){const title={proposal:'Проверка коммерческого предложения',worklist:'Проверка перечня работ',act:'Проверка акта'}[documentContext.type]||'Проверка расчёта';openSheet(`<div class="sectionTitle"><div><h1>${title}</h1><p class="help compact">Подсказки мастера не попадут в PDF.</p></div><button class="btn ghost small" data-close-sheet>Закрыть</button></div><div id="reviewRows" class="rowList">${reviewRows.length?reviewRows.map(reviewRowHtml).join(''):'<div class="empty">Позиции не добавлены.</div>'}</div><button id="addReviewRow" class="btn ghost wide" style="margin-top:9px">＋ Добавить работу</button><div class="reviewSummary" style="margin-top:13px"><span>Итого документа</span><strong id="reviewTotal">${money(reviewRows.reduce((s,r)=>s+parseNum(r.qty)*parseNum(r.price),0))}</strong></div><div class="actions end"><button id="generateReviewedDoc" class="btn primary">Сформировать документ</button></div>`);
  $$('[data-review-index]',$('sheetPanel')).forEach(el=>el.addEventListener(el.tagName==='SELECT'?'change':'input',()=>{const row=reviewRows[+el.dataset.reviewIndex],key=el.dataset.reviewKey;row[key]=['qty','price'].includes(key)?rawNum(el.value):el.value;const sum=$('sheetPanel').querySelector(`[data-review-sum="${el.dataset.reviewIndex}"]`);if(sum)sum.textContent=money(parseNum(row.qty)*parseNum(row.price));$('reviewTotal').textContent=money(reviewRows.reduce((s,r)=>s+parseNum(r.qty)*parseNum(r.price),0))}));
  ...
  $('generateReviewedDoc').onclick=async()=>{const {type,options}=documentContext;closeSheet();buildOrderDocument(type,reviewRows,options);const order=currentOrder();if(order){order.documentHistory.push({id:uid(),type,date:now(),total:reviewRows.reduce((s,r)=>s+parseNum(r.qty)*parseNum(r.price),0),snapshot:reviewRows.map(clone)});await saveObject(currentObject())}}
}
function groupedRowsTable(rows){const groups=[];for(const row of rows)if(!groups.includes(row.group||'Работы'))groups.push(row.group||'Работы');return groups.map(group=>`<tr class="group"><td colspan="5">${esc(group)}</td></tr>`+rows.filter(r=>(r.group||'Работы')===group).map(r=>`<tr><td>${esc(r.name)}${r.comment?`<br><small>${esc(r.comment)}</small>`:''}</td><td>${qty(r.qty)}</td><td>${esc(r.unit)}</td><td>${money(r.price)}</td><td>${money(parseNum(r.qty)*parseNum(r.price))}</td></tr>`).join('')).join('')}
...
function buildOrderDocument(type,rows,options={}){const object=currentObject(),order=currentOrder();if(!object||!order)return;const workTotal=rows.filter(r=>r.kind!=='material').reduce((s,r)=>s+parseNum(r.qty)*parseNum(r.price),0),materialTotal=rows.filter(r=>r.kind==='material').reduce((s,r)=>s+parseNum(r.qty)*parseNum(r.price),0),total=workTotal+materialTotal;const titles={proposal:'Коммерческое предложение',worklist:'Перечень выполненных работ',act:'Акт выполненных работ'};let html=paperHeader(titles[type]||'Документ',{slogan:type==='proposal'});html+=infoGrid([['Заказчик',object.contact.name],['Телефон',formatPhone(object.contact.phone)],['Адрес объекта',object.contact.address],['Заказ',order.title],['Дата',ruDate(order.date||today())]]);html+=`<h2>Работы и услуги</h2><div style="overflow-x:auto"><table><thead><tr><th>Наименование</th><th>Количество</th><th>Ед.</th><th>Цена</th><th>Сумма</th></tr></thead><tbody>${groupedRowsTable(rows)}</tbody></table></div>`;
  if(type==='proposal')html+=`<div class="paperTotal"><div><span>Стоимость работ</span><strong>${money(workTotal)}</strong></div>${materialTotal?`<div><span>Материалы для заказчика</span><strong>${money(materialTotal)}</strong></div>`:''}<div class="grand"><span>Итого</span><strong>${money(total)}</strong></div></div>${options.procurement==='recommend'?`<div class="paperAlso"><strong>Организация закупки материалов</strong><p>Подбор, согласование и организация приобретения материалов. Ориентировочная стоимость услуги ${money(options.procurementPrice)}. Материалы и доставка оплачиваются отдельно.</p></div>`:''}<p class="paperNote">Расчёт составлен по указанному объёму и условиям. Дополнительные или скрытые работы выполняются только после согласования с заказчиком.</p>`;
  if(type==='worklist')html+=`<div class="paperTotal"><div class="grand"><span>Итого выполненных работ</span><strong>${money(workTotal)}</strong></div></div><p class="paperNote">Перечень отражает фактически выполненный объём и стоимость работ по объекту.</p>`;
  if(type==='act')html+=`<div class="paperTotal"><div class="grand"><span>Стоимость принятых работ</span><strong>${money(workTotal)}</strong></div></div><p class="paperNote">Заказчик подтверждает, что перечисленные работы выполнены в полном объёме, результат принят, претензий по объёму и качеству работ не имеется. При наличии замечаний они указываются ниже до подписания акта.</p><div class="blankLines"><div class="blankLine"><span>Заказчик, ФИО</span><span></span></div><div class="blankLine"><span>Замечания</span><span></span></div></div><div class="signature"><div>Исполнитель / подпись</div><div>Заказчик / подпись</div></div>`;
...
function buildContract(order,options){const object=order?currentObject():null,includeMaster=options.masterMode==='profile',includeOrder=!!order&&options.orderMode==='include',date=options.dateMode==='today'?ruDate(today()):'________________';const fio=blankOr(profileFullName(),includeMaster),passport=includeMaster&&profile.passportSeries&&profile.passportNumber?`${esc(profile.passportSeries)} ${esc(profile.passportNumber)}`:'&nbsp;',issued=blankOr(profile.passportIssuedBy,includeMaster),issuedDate=includeMaster&&profile.passportIssuedDate?ruDate(profile.passportIssuedDate):'&nbsp;',code=blankOr(profile.passportCode,includeMaster),registration=blankOr(profile.registrationAddress,includeMaster),phone=blankOr(formatPhone(profile.phone),includeMaster);const work=order?orderWorkTotal(order):0;const workRows=order?[...calculateFloor(order.floor).rows,...calculateDoors(order.doors).rows]:[];const list=includeOrder&&workRows.length?`<ol>${workRows.map(r=>`<li>${esc(r.name)} · ${qty(r.qty)} ${esc(r.unit)}</li>`).join('')}</ol>`:`<div class="blankLines"><div class="blankLine"><span>Перечень работ</span><span></span></div><div class="blankLine"><span></span><span></span></div><div class="blankLine"><span></span><span></span></div></div>`;const one=`${paperHeader('Договор на выполнение работ')}<p style="text-align:right">г. ${esc(profile.city||'____________')} · ${date}</p><div class="contractText"><p><strong>Исполнитель</strong> ${fio} и <strong>Заказчик</strong> __________________________________________ заключили настоящий договор о нижеследующем.</p><h2>1. Предмет договора</h2><p>Исполнитель обязуется выполнить согласованные работы, а Заказчик принять результат и произвести оплату.</p>${includeOrder&&object?infoGrid([['Адрес объекта',object.contact.address],['Заказ',order.title],['Согласованная стоимость',work?money(work):'']]):''}${list}<h2>2. Стоимость и порядок оплаты</h2><p>Стоимость работ определяется согласованным перечнем и объёмом. Работы, не включённые в первоначальный перечень, выполняются после отдельного согласования стоимости.</p><div class="blankLines"><div class="blankLine"><span>Стоимость работ</span><span>${includeOrder&&work?money(work):''}</span></div><div class="blankLine"><span>Порядок оплаты</span><span></span></div></div><h2>3. Сроки и условия</h2><p>Срок начала и завершения работ, доступ на объект, материалы и иные условия согласовываются сторонами. Обстоятельства, обнаруженные после начала работ и влияющие на объём или технологию, фиксируются и согласовываются отдельно.</p><h2>4. Сдача и приёмка</h2><p>После завершения работ стороны подписывают акт. Замечания при наличии указываются до подписания акта.</p><h2>5. Реквизиты сторон</h2><div class="grid two"><div><strong>Исполнитель</strong><div class="blankLines"><div class="blankLine"><span>ФИО</span><span>${fio}</span></div><div class="blankLine"><span>Паспорт</span><span>${passport}</span></div><div class="blankLine"><span>Выдан</span><span>${issued}</span></div><div class="blankLine"><span>Дата выдачи</span><span>${issuedDate}</span></div><div class="blankLine"><span>Код</span><span>${code}</span></div><div class="blankLine"><span>Регистрация</span><span>${registration}</span></div><div class="blankLine"><span>Телефон</span><span>${phone}</span></div></div></div><div><strong>Заказчик</strong><div class="blankLines"><div class="blankLine"><span>ФИО</span><span></span></div><div class="blankLine"><span>Паспорт</span><span></span></div><div class="blankLine"><span>Выдан</span><span></span></div><div class="blankLine"><span>Дата выдачи</span><span></span></div><div class="blankLine"><span>Код</span><span></span></div><div class="blankLine"><span>Регистрация</span><span></span></div><div class="blankLine"><span>Телефон</span><span></span></div></div></div></div><div class="signature"><div>Исполнитель / подпись</div><div>Заказчик / подпись</div></div></div>`;showPaper(Array.from({length:options.copies||1},(_,i)=>`<section class="printCopy">${one}${options.copies>1?`<p class="paperNote">Экземпляр ${i+1} из ${options.copies}</p>`:''}</section>`).join('<div style="break-after:page"></div>'))}

function showPriceListSheet(){const saved=(()=>{try{return JSON.parse(storageGet('framePriceSectionsV170','{}')||'{}')}catch(e){return {}}})();const sections={floorCover:saved.floorCover!==false,floorBaseboard:!!saved.floorBaseboard,floorPrep:!!saved.floorPrep,doorsInstall:!!saved.doorsInstall,doorsService:!!saved.doorsService,doorsOpening:!!saved.doorsOpening};openSheet(`<div class="sectionTitle"><div><h1>Прайс-лист</h1><p class="help compact">Отметьте только то, что клиент запросил. Остальное попадёт в короткий блок «Также выполняем».</p></div><button class="btn ghost small" data-close-sheet>Закрыть</button></div><h3>Полы</h3><div class="checkline"><label class="check"><input data-price-section="floorCover" type="checkbox" ${sections.floorCover?'checked':''}> Напольные покрытия</label><label class="check"><input data-price-section="floorBaseboard" type="checkbox" ${sections.floorBaseboard?'checked':''}> Плинтусы</label><label class="check"><input data-price-section="floorPrep" type="checkbox" ${sections.floorPrep?'checked':''}> Подготовка и демонтаж</label></div><h3>Двери</h3><div class="checkline"><label class="check"><input data-price-section="doorsInstall" type="checkbox" ${sections.doorsInstall?'checked':''}> Установка дверей</label><label class="check"><input data-price-section="doorsService" type="checkbox" ${sections.doorsService?'checked':''}> Обслуживание</label><label class="check"><input data-price-section="doorsOpening" type="checkbox" ${sections.doorsOpening?'checked':''}> Проёмы</label></div><div class="actions end"><button id="generatePriceList" class="btn primary">Сформировать PDF</button></div>`);$('generatePriceList').onclick=()=>{const selected={};$$('[data-price-section]',$('sheetPanel')).forEach(x=>selected[x.dataset.priceSection]=x.checked);if(!Object.values(selected).some(Boolean)){toast('Выберите хотя бы один раздел');return}storageSet('framePriceSectionsV170',JSON.stringify(selected));closeSheet();buildPriceList(selected)}}
function rateTable(title,items){const visible=items.filter(([,key])=>parseNum(rates[key])>0);if(!visible.length)return '';return `<h2>${esc(title)}</h2><table><thead><tr><th>Работа</th><th>Цена</th></tr></thead><tbody>${visible.map(([label,key,unit='м²'])=>`<tr><td>${esc(label)}</td><td>${money(rates[key])}/${esc(unit)}</td></tr>`).join('')}</tbody></table>`}
function buildPriceList(selected){let body='';if(selected.floorCover)body+=RATE_GROUPS.floor.slice(0,4).map(g=>rateTable(g.title,g.items)).join('');if(selected.floorBaseboard)body+=rateTable('Плинтус',RATE_GROUPS.floor[4].items);if(selected.floorPrep)body+=rateTable('Подготовка основания',RATE_GROUPS.floor[5].items)+rateTable('Демонтаж и дополнительные работы',RATE_GROUPS.floor[6].items);if(selected.doorsInstall)body+=rateTable('Установка дверей',RATE_GROUPS.doors[0].items);if(selected.doorsService)body+=rateTable('Обслуживание и доработка дверей',RATE_GROUPS.doors[1].items);if(selected.doorsOpening)body+=rateTable('Проёмы и примыкания',RATE_GROUPS.doors[2].items);const also=[];if(!selected.floorCover)also.push('укладку напольных покрытий');if(!selected.floorBaseboard)also.push('монтаж плинтуса');if(!selected.floorPrep)also.push('демонтаж и подготовку основания');if(!selected.doorsInstall)also.push('установку дверей');if(!selected.doorsService)also.push('обслуживание и регулировку дверей');if(!selected.doorsOpening)also.push('оформление дверных проёмов');const html=`${paperHeader('Прайс-лист',{slogan:true})}${infoGrid([['Мастер',profileFullName()],['Телефон',formatPhone(profile.phone)],['Город',profile.city],['Дата',ruDate(today())]])}${body}<div class="paperAdvantages"><div class="paperAdvantage"><strong>Работаем самостоятельно</strong><br>Без посредников и передачи заказа сторонним исполнителям.</div><div class="paperAdvantage"><strong>Согласовываем заранее</strong><br>Состав и изменение стоимости обсуждаются до выполнения работ.</div><div class="paperAdvantage"><strong>Прямой контакт</strong><br>На вопросы отвечает человек, который выполняет работу.</div><div class="paperAdvantage"><strong>Работаем без лишних простоев</strong><br>Сохраняем необходимую технологию и отвечаем за результат.</div></div>${also.length?`<div class="paperAlso"><strong>Также выполняем</strong><p>${esc(also.join(', '))}.</p></div>`:''}<p class="paperNote"><strong>Цены в прайс-листе являются ориентировочными.</strong> Окончательная стоимость определяется после уточнения объёма, материала, состояния основания, сложности работ и условий объекта и согласовывается до начала работ. Точный расчёт по конкретному объекту имеет приоритет над общим прайс-листом.</p>`;showPaper(html)}
function showPaper(html){$('paper').innerHTML=html;$('documentView').classList.remove('hidden');window.scrollTo({top:0,behavior:'auto'})}

function exportBackup(){const includePrivate=storageGet(EXPORT_PRIVATE_KEY,'0')==='1';const safeProfile=clone(profile);if(!includePrivate)for(const key of ['passportSeries','passportNumber','passportIssuedBy','passportIssuedDate','passportCode','registrationAddress'])safeProfile[key]='';const data={format:'FRAME_BACKUP',version:VERSION,exportedAt:now(),profile:safeProfile,rates,objects:objects.map(stripPhotos),privateDataIncluded:includePrivate};const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`FRAME-backup-${today()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);toast('Резервная копия подготовлена')}
async function handleImportFile(event){const file=event.target.files?.[0];event.target.value='';if(!file)return;try{const data=JSON.parse(await file.text()),arr=Array.isArray(data)?data:data.objects;if(!Array.isArray(arr))throw new Error('objects');importCandidate={objects:arr.map(normalizeObject),rates:data.rates&&typeof data.rates==='object'?data.rates:null,profile:data.profile&&typeof data.profile==='object'?data.profile:null};showImportPreview()}catch(e){console.error(e);toast('Не удалось прочитать резервную копию')}}
...
async function applyImport(mode){if(!importCandidate)return;try{for(const incoming of importCandidate.objects){const object=clone(incoming);if(mode==='new'){object.id=uid();object.orders.forEach(o=>o.id=uid())}await dbPut(normalizeObject(object))}if(importCandidate.rates){rates={...rates,...importCandidate.rates};saveRates()}if(importCandidate.profile){const incoming=importCandidate.profile;profile={...profile,...incoming};saveProfile()}await reloadObjects();importCandidate=null;closeSheet();navigate('dashboard');toast('Импорт завершён')}catch(e){console.error(e);toast('Ошибка импорта')}}
