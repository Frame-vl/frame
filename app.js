...
function normalizeExpense(e={}){return {id:e.id||uid(),category:e.category||'other',amount:String(e.amount??''),date:e.date||today(),worker:String(e.worker||''),comment:String(e.comment||'')}}
function loadGeneralExpenses(){try{const arr=JSON.parse(storageGet(GENERAL_EXPENSES_KEY,'[]')||'[]');return Array.isArray(arr)?arr.map(normalizeExpense):[]}catch(e){return []}}
...
function defaultOrder(index=1){return {id:uid(),title:`Заказ ${index}`,date:today(),status:'draft',comment:'',floor:defaultFloor(),doors:defaultDoors(),purchases:[],expenses:[],payments:[],stages:[],photos:[],documentHistory:[]}}
...
function normalizeOrder(raw={},index=1){return {
  ...
  floor:normalizeFloor(raw.floor||{}),doors:normalizeDoors(raw.doors||{}),purchases:(raw.purchases||[]).map(normalizePurchase),expenses:(raw.expenses||[]).map(normalizeExpense),
...
function expenseCategoryName(value){return {worker:'Оплата работнику',fuel:'Бензин и дорога',tool:'Инструмент',consumables:'Расходники за свой счёт',delivery:'Доставка',rent:'Аренда оборудования',other:'Прочее'}[value]||'Прочее'}
...
function navigate(next,state={},options={}){closeSheet();route=next;routeState=state;render();requestAnimationFrame(()=>{document.documentElement.scrollLeft=0;document.body.scrollLeft=0;if(options.keepScroll)return;if(options.scrollToId){const el=$(options.scrollToId);if(el)el.scrollIntoView({block:'start',inline:'nearest',behavior:options.behavior||'smooth'});else window.scrollTo({left:0,top:0,behavior:'auto'})}else window.scrollTo({left:0,top:0,behavior:options.behavior||'auto'})})}
...
function renderDashboard(){
  const total=objects.reduce((sum,o)=>sum+objectTotals(o).work,0),active=objects.filter(o=>o.orders.some(r=>['agreed','work'].includes(r.status))).length;
  ...
  return `<section class="view">
    <div class="card"><h1>Рабочие объекты</h1><p class="help">Локальный кабинет мастера. Контакты, расчёты и документы остаются на этом устройстве.</p>
      ...
      <button id="financeCard" class="financeCard" type="button"><span><strong>${esc(range.label)}</strong><small>Нажмите, чтобы выбрать период</small></span><span><small>Получено</small><strong>${money(finance.received)}</strong></span><span><small>Расходы</small><strong>${money(finance.spent)}</strong></span><span><small>Чистыми</small><strong>${money(finance.net)}</strong></span></button>
...
function renderOrderView(){
  ...
  const work=orderWorkTotal(order),paid=orderPaid(order),remaining=orderRemaining(order),due=orderDuePurchases(order),expenses=orderExpenses(order),profit=orderProfit(order);
  return `<section class="view">
...
function renderFinancesView(){
  ...
  for(const expense of data.expenses){
    ...
    if(expense.worker)workers[expense.worker]=(workers[expense.worker]||0)+parseNum(expense.amount);
  ...
  for(const payment of data.payments){
    const key=payment.orderId||payment.orderTitle;
    orders[key]=orders[key]||{title:payment.orderTitle,address:payment.address,received:0,spent:0};
    orders[key].received+=parseNum(payment.amount);
  }
  return `<section class="view"><div class="actions pageBack"><button class="btn ghost" data-go="dashboard">← Объекты</button></div><div class="card">
    <div class="sectionTitle financeTitle"><div><h1>Финансы</h1><p class="help compact">Сколько пришло, сколько потрачено и сколько осталось за выбранный период.</p></div><button id="financePeriodBtn" class="btn ghost periodButton">${esc(range.label)} ▾</button></div>
    <div class="financeMetrics"><div class="moneyMetric primary"><span>Получено</span><strong>${money(data.received)}</strong></div><div class="moneyMetric"><span>Расходы</span><strong>${money(data.spent)}</strong></div><div class="moneyMetric"><span>Чистыми</span><strong>${money(data.net)}</strong></div><div class="moneyMetric"><span>К возмещению</span><strong>${money(data.due)}</strong></div></div>
    <details open><summary>По заказам</summary><div class="summaryList">${Object.values(orders).length?Object.values(orders).sort((a,b)=>(b.received-b.spent)-(a.received-a.spent)).map(item=>`<div class="financeOrderLine"><span><strong>${esc(item.title)}</strong><small>${esc(item.address||'Без адреса')}</small></span><span><small>Получено ${money(item.received)}</small><strong>${money(item.received-item.spent)}</strong></span></div>`).join(''):'<div class="empty">Движений по заказам в этом периоде нет.</div>'}</div></details>
    ...
    <details><summary>Все расходы</summary><div class="summaryList">${data.expenses.length?data.expenses.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(expense=>`<div class="summaryLine"><span>${esc(expenseCategoryName(expense.category))}<small class="muted" style="display:block">${esc(expense.orderTitle)} · ${ruDate(expense.date)}</small></span><strong>${money(expense.amount)}</strong></div>`).join(''):'<div class="empty">Записей нет.</div>'}</div></details>
    <h2>Общие расходы</h2><p class="help compact">Не привязаны к одному объекту, например инструмент или общая поездка.</p><div class="rowList">${expenseRowsHtml(generalExpenses,'general')}</div><button id="addGeneralExpenseBtn" class="btn ghost wide" style="margin-top:10px">＋ Добавить общий расход</button>
  </div></section>`;
}
...
function bindRoute(){
  ...
  if(route==='dashboard'){
    ...
    $('financeCard').onclick=()=>navigate('finances');
...
function bindObjectView(){const object=currentObject();if(!object)return;
  ...
  $('newOrderBtn').onclick=()=>{const order=defaultOrder(object.orders.length+1);object.orders.push(order);currentOrderId=order.id;queueSave();navigate('order',{}, {scrollToId:'orderActionsCard',behavior:'smooth'});toast('Новый заказ создан')};
...
function bindExpenseRows(scope,list){$$(`[data-expense-scope="${scope}"]`).forEach(el=>el.addEventListener(el.tagName==='SELECT'||el.type==='date'?'change':'input',()=>{const item=list[+el.dataset.expenseIndex];if(!item)return;const key=el.dataset.expenseKey;item[key]=key==='amount'?rawNum(el.value):el.value;if(scope==='general')saveGeneralExpenses();else queueSave();if($('expenseTotal'))$('expenseTotal').textContent=money(orderExpenses(currentOrder()))}));$$(`[data-remove-expense-scope="${scope}"]`).forEach(b=>b.onclick=()=>{list.splice(+b.dataset.removeExpense,1);if(scope==='general')saveGeneralExpenses();else queueSave();render()})}
function bindExpensesView(){const order=currentOrder();if(!order)return;bindExpenseRows('order',order.expenses);$('addOrderExpenseBtn').onclick=()=>{order.expenses.push(normalizeExpense({date:today()}));queueSave();render();requestAnimationFrame(()=>window.scrollTo({left:0,top:document.body.scrollHeight,behavior:'smooth'}))}}
function bindFinancesView(){const saved=(()=>{try{return JSON.parse(storageGet(FINANCE_PERIOD_KEY,'{}')||'{}')}catch(e){return {}}})();$('financePeriodBtn').onclick=showFinancePeriodSheet;bindExpenseRows('general',generalExpenses);$('addGeneralExpenseBtn').onclick=()=>{generalExpenses.push(normalizeExpense({date:today()}));saveGeneralExpenses();render();requestAnimationFrame(()=>window.scrollTo({left:0,top:document.body.scrollHeight,behavior:'smooth'}))}}
...
function showPaper(html){$('paper').innerHTML=html;$('documentView').classList.remove('hidden');window.scrollTo({left:0,top:0,behavior:'auto'})}
