...
function Find-NumberRecursive([object]$Value, [double]$Expected) {
    ...
    if ($Value -is [System.Collections.IDictionary]) { foreach ($v in $Value.Values) { if (Find-NumberRecursive $v $Expected) { return $true } }; return $false }
    if ($Value -is [System.Collections.IEnumerable]) { foreach ($v in $Value) { if (Find-NumberRecursive $v $Expected) { return $true } }; return $false }
...
function Check-Expect([object]$Data, [object]$Expect) {
    ...
    foreach ($x in @($Expect.must_reference)) { if ($x -and -not $text.Contains((Normalize $x))) { $errors.Add("missing expected fragment: $x") } }
    $anyProp = $Expect.PSObject.Properties['must_reference_any']
    if ($anyProp -and $anyProp.Value) {
        $foundAny = $false
        foreach ($x in @($anyProp.Value)) { if ($x -and $text.Contains((Normalize $x))) { $foundAny = $true; break } }
        if (-not $foundAny) { $errors.Add("missing all equivalent expected fragments: $(@($anyProp.Value) -join ' OR ')") }
    }
    foreach ($x in @($Expect.must_not_reference)) { if ($x -and $text.Contains((Normalize $x))) { $errors.Add("forbidden fragment found: $x") } }
    $actions = @()
    ...
    foreach ($field in @('quantity','unit_price','total')) {
        ...
        if ($p -and $actions.Count -gt 0 -and -not (Find-NumberRecursive $actions ([double]$p.Value))) { $errors.Add("numeric value not found: $field=$($p.Value)") }
    }
    return $errors
}
...
foreach ($sc in $suite.scenarios) {
    try {
        ...
        foreach ($msg in $sc.messages) {
            ...
            $context.current_target = $target; $context.conversation_target = $target
            $context | Add-Member -NotePropertyName conversation -NotePropertyValue $conversation -Force
            $context | Add-Member -NotePropertyName conversation_rules -NotePropertyValue @('Stay on conversation_target until the user explicitly names another object.','Recent user facts in conversation are newer than stored progress until actions are applied.','When recent user statements conflict, the newest explicit statement is authoritative and replaces the older fact.','A clear correction or negation is not ambiguous and must not trigger a clarification question.','Never switch to another object/order merely because it exists in context.') -Force
        ...
        if ($errors.Count -eq 0) { $passed++; Write-Host "PASS $($sc.id)" } else { Write-Host "FAIL $($sc.id)"; foreach ($e in $errors) { Write-Host "  - $e" }; Write-Host "  response: $($last | ConvertTo-Json -Depth 20 -Compress)" }
    } catch { Write-Host "ERROR $($sc.id)"; Write-Host "  - $($_.Exception.Message)" }
}
Write-Host ''
