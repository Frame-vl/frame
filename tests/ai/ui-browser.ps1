...
$stdout = Join-Path $env:TEMP ('frame-edge-ui-out-' + [guid]::NewGuid().ToString('N') + '.txt')
$stderr = Join-Path $env:TEMP ('frame-edge-ui-err-' + [guid]::NewGuid().ToString('N') + '.txt')
...
try {
  ...
  if ($text -notmatch 'FRAME_UI_E2E_PASS') {
    Write-Host '--- FRAME UI browser DOM ---'
    Write-Host $text
    Write-Host '--- end DOM ---'
